function FEMdata = LS3DB_interp_funs(FEMdata)
...
if FEMdata.warp_DOF
    ...
    f_dist_l = @(xi,fa,ql,qt,tq,bm) [fa(xi); ql(xi); qt(xi); tq(xi); qt(xi); -ql(xi); bm(xi)];
    f_dist_g = @(xi,fx,qy,qz,mx) [fx(xi); qy(xi); qz(xi); mx(xi); qz(xi); -qy(xi); 0];
    % Shape functions for interpolation of concentraded loads/sources
    ...
    % Strain-displacement (B) matrices
    B = @(i,xi,h_e,J,Gamx,Gamy,Gamz) [J^-1*dzeta{i}(xi) 0                                                               0                                                               0                   0                                                                                                                      0                                               
                                      0                 phi{2*i-1}(xi,h_e,Gamy(xi))+J^-1*dpsi{2*i-1}(xi,h_e,Gamy(xi))   0                                                               0                   0                                                           -(phi{2*i}(xi,h_e,Gamy(xi))+J^-1*dpsi{2*i}(xi,h_e,Gamy(xi)))                        
                                      0                 0                                                               phi{2*i-1}(xi,h_e,Gamz(xi))+J^-1*dpsi{2*i-1}(xi,h_e,Gamz(xi))   0                   phi{2*i}(xi,h_e,Gamz(xi))+J^-1*dpsi{2*i}(xi,h_e,Gamz(xi))                                                              0                                              
    ...
    f_dist_l = @(xi,fa,ql,qt,tq,bm) [fa(xi); ql(xi); qt(xi); tq(xi); qt(xi); -ql(xi)];
    f_dist_g = @(xi,fx,qy,qz,mx) [fx(xi); qy(xi); qz(xi); mx(xi); qz(xi); -qy(xi)];
    % Shape functions for interpolation of concentraded loads/sources
    H_psi = @(i,xi,h_e,Gam) [psi{2*i-1}(xi,h_e,Gam(xi)); psi{2*i}(xi,h_e,Gam(xi))];
    H_phi = @(i,xi,h_e,Gam) [phi{2*i-1}(xi,h_e,Gam(xi)); phi{2*i}(xi,h_e,Gam(xi))];
    H_zeta = @(i,xi) zeta{i}(xi);
...
FEMdata.funs.H_dist = H_dist;
FEMdata.funs.H_psi = H_psi;
FEMdata.funs.H_phi = H_phi;
FEMdata.funs.H_zeta = H_zeta;
FEMdata.funs.f_dist_l = f_dist_l;
FEMdata.funs.f_dist_g = f_dist_g;
