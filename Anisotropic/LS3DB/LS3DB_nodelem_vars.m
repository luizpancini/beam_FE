function [FEMdata,edata] = LS3DB_nodelem_vars(FEMdata,edata,fx_of_x,mx_of_x,qy_of_x,qz_of_x,fa_of_x,ql_of_x,qt_of_x,tq_of_x,bm_of_x,Px_of_x,Py_of_x,Pz_of_x,Pa_of_x,Pl_of_x,Pt_of_x,Mx_of_x,My_of_x,Mz_of_x,Ml_of_x,Mt_of_x,Tq_of_x,Bm_of_x,CSx_of_x,CSy_of_x,CSz_of_x,CSu_of_x,CSv_of_x,CSw_of_x,CSt_of_x,CSbl_of_x,CSbt_of_x,cfl_of_x,cft_of_x)

% Unpack FEM data
[props,warp_DOF,L,b_alpha,b_beta,b_gamma,Ne_b,element_order,elem_connect,elem_nodes,apx,apy,apz,amx,amy,amz,apa,apl,apt,atq,aml,amt,abm,akx,aky,akz,aku,akv,akw,akt,akbl,akbt] = unpack_FEMdata(FEMdata,'nodelem_vars');
...
indexat = @(expr, index) expr(index);   % To get specific array index
indicesat = @(mat,i,j) mat(i,j);        % To get specific matrix index
% Loop over elements
for e=1:Ne
    ...
    if props.constitutive_model{beam{e}} == "iso"
        e_props{e}.E_of_x = @(x) indexat(props.E(x),beam{e});
        e_props{e}.G_of_x = @(x) indexat(props.G(x),beam{e});
        e_props{e}.A_of_x = @(x) indexat(props.A(x),beam{e});
        e_props{e}.J_of_x = @(x) indexat(props.J(x),beam{e});
        e_props{e}.Gamma_of_x = @(x) indexat(props.Gamma(x),beam{e});
        e_props{e}.Iyy_of_x = @(x) indexat(props.Iyy(x),beam{e});
        e_props{e}.Izz_of_x = @(x) indexat(props.Izz(x),beam{e});
        e_props{e}.Ksy_of_x = @(x) indexat(props.Ksy(x),beam{e});
        e_props{e}.Ksz_of_x = @(x) indexat(props.Ksz(x),beam{e});
        e_props{e}.Gamx_of_x = @(x) 0;
        e_props{e}.Gamy_of_x = @(x) e_props{e}.E_of_x(x).*e_props{e}.Izz_of_x(x)./(e_props{e}.Ksy_of_x(x).*e_props{e}.G_of_x(x).*e_props{e}.A_of_x(x)*L0{e}^2);
        e_props{e}.Gamz_of_x = @(x) e_props{e}.E_of_x(x).*e_props{e}.Iyy_of_x(x)./(e_props{e}.Ksz_of_x(x).*e_props{e}.G_of_x(x).*e_props{e}.A_of_x(x)*L0{e}^2);
    elseif props.constitutive_model{beam{e}} == "aniso"
        e_props{e}.C_aniso_of_x = @(x) props.C_aniso{beam{e}}(x);
        C22 = @(x) indicesat(props.C_aniso{beam{e}}(x),2,2);
        C33 = @(x) indicesat(props.C_aniso{beam{e}}(x),3,3);
        C55 = @(x) indicesat(props.C_aniso{beam{e}}(x),5,5);
        C66 = @(x) indicesat(props.C_aniso{beam{e}}(x),6,6);
        e_props{e}.Gamx_of_x = @(x) 0;
        e_props{e}.Gamy_of_x = @(x) C66(x)./(C22(x)*L0{e}^2);
        e_props{e}.Gamz_of_x = @(x) C55(x)./(C33(x)*L0{e}^2);
    end
    % Element constitutive properties as functions of the parent coordinate xi
    x = @(xi) L0{e}/2*(1+xi)+x1{e}; 
    if props.constitutive_model{beam{e}} == "iso"
        e_props{e}.E_of_xi = @(xi) indexat(props.E(x(xi)),beam{e});
        e_props{e}.G_of_xi = @(xi) indexat(props.G(x(xi)),beam{e});
        e_props{e}.A_of_xi = @(xi) indexat(props.A(x(xi)),beam{e});
        e_props{e}.J_of_xi = @(xi) indexat(props.J(x(xi)),beam{e});
        e_props{e}.Gamma_of_xi = @(xi) indexat(props.Gamma(x(xi)),beam{e});
        e_props{e}.Iyy_of_xi = @(xi) indexat(props.Iyy(x(xi)),beam{e});
        e_props{e}.Izz_of_xi = @(xi) indexat(props.Izz(x(xi)),beam{e});
        e_props{e}.Ksy_of_xi = @(xi) indexat(props.Ksy(x(xi)),beam{e});
        e_props{e}.Ksz_of_xi = @(xi) indexat(props.Ksz(x(xi)),beam{e});
        e_props{e}.Gamx_of_xi = @(xi) 0;
        e_props{e}.Gamy_of_xi = @(xi) e_props{e}.E_of_xi(xi).*e_props{e}.Izz_of_xi(xi)./(e_props{e}.Ksy_of_xi(xi).*e_props{e}.G_of_xi(xi).*e_props{e}.A_of_xi(xi)*L0{e}^2);
        e_props{e}.Gamz_of_xi = @(xi) e_props{e}.E_of_xi(xi).*e_props{e}.Iyy_of_xi(xi)./(e_props{e}.Ksz_of_xi(xi).*e_props{e}.G_of_xi(xi).*e_props{e}.A_of_xi(xi)*L0{e}^2);
    elseif props.constitutive_model{beam{e}} == "aniso"
        e_props{e}.C_aniso_of_xi = @(xi) props.C_aniso{beam{e}}(x(xi));
        C22 = @(xi) indicesat(props.C_aniso{beam{e}}(x(xi)),2,2);
        C33 = @(xi) indicesat(props.C_aniso{beam{e}}(x(xi)),3,3);
        C55 = @(xi) indicesat(props.C_aniso{beam{e}}(x(xi)),5,5);
        C66 = @(xi) indicesat(props.C_aniso{beam{e}}(x(xi)),6,6);
        e_props{e}.Gamx_of_xi = @(xi) 0;
        e_props{e}.Gamy_of_xi = @(xi) C66(x(xi))./(C22(x(xi))*L0{e}^2);
        e_props{e}.Gamz_of_xi = @(xi) C55(x(xi))./(C33(x(xi))*L0{e}^2);
    end
