...
% Unpack FEM data
[beam_theory,warp_DOF,props,scale,Ne,enn,n_div,DOF_u,DOF_v,DOF_w,DOF_phix,DOF_phiy,DOF_phiz,DOF_dphix,psi,dpsi,d2psi,d3psi,phi,dphi,d2phi,zeta,dzeta] = unpack_FEMdata(FEMdata,'outputs');
...
for e=1:Ne
    ...
    %% Displacements and derivatives - as functions of the local beam coordinate x
    % Unpack element data in the local coordinate x    
    [e_dof_range,L0,x1,beam,R0,T0,Jac,x_vec_nodes,x_vec_interp,x0,y0,z0,e_props,Gamx,Gamy,Gamz] = unpack_edata(edata,e,'outputs');  
    ...
    %% Local internal loads - as functions of the local beam coordinate x
    if props.constitutive_model{beam} == "iso"
        % Rename props
        E = @(x) e_props.E_of_x(x);
        A = @(x) e_props.A_of_x(x);
        G = @(x) e_props.G_of_x(x);
        J = @(x) e_props.J_of_x(x);
        Iyy = @(x) e_props.Iyy_of_x(x);
        Izz = @(x) e_props.Izz_of_x(x);
        Ksy = @(x) e_props.Ksy_of_x(x);
        Ksz = @(x) e_props.Ksz_of_x(x);
        Gamma = @(x) e_props.Gamma_of_x(x);
        % Set loads according to theory
        ...
        if warp_DOF
            ...
            Bm_interp_fun = @(x) zeros(length(x),1);
        end
    elseif props.constitutive_model{beam} == "aniso"
        % Current element's constitutive stiffness matrix
        rowat = @(mat,row) mat(row,:);
        indexat = @(mat,i,j) mat(i,j);
        elem_C = @(x) e_props.C_aniso_of_x(x);
        % Set strains accodring to theory
        gamma_x = @(x) du_interp_fun(x);
        kappa_x = @(x) dphix_interp_fun(x);
        kappa_x_prime = @(x) d2phix_interp_fun(x);
        if beam_theory == "EB"
        ...
        % Set strains vector
        if warp_DOF
            d_interp_fun = @(x) [gamma_x(x)'; gamma_y(x)'; gamma_z(x)'; kappa_x(x)'; kappa_y(x)'; kappa_z(x)'; kappa_x_prime(x)'];
        else
            d_interp_fun = @(x) [gamma_x(x)'; gamma_y(x)'; gamma_z(x)'; kappa_x(x)'; kappa_y(x)'; kappa_z(x)'];
        end
        % Set loads
        Nx_interp_fun = @(x) (rowat(elem_C(x),1)*d_interp_fun(x))';
        Vy_interp_fun = @(x) (rowat(elem_C(x),2)*d_interp_fun(x))';
        Vz_interp_fun = @(x) (rowat(elem_C(x),3)*d_interp_fun(x))';
        Tq_interp_fun = @(x) (rowat(elem_C(x),4)*d_interp_fun(x))';
        My_interp_fun = @(x) (rowat(elem_C(x),5)*d_interp_fun(x))';
        Mz_interp_fun = @(x) (rowat(elem_C(x),6)*d_interp_fun(x))';
        if warp_DOF
            Bm_interp_fun = @(x) (rowat(elem_C(x),7)*d_interp_fun(x))';
        else
            Bm_interp_fun = @(x) zeros(length(x),1);
        end
    end
