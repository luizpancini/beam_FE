...
% Check longitudinal vibration of clamped-tipmass beam
...
Ma{1} = m0; ama{1} = L0; MaRix{1} = 0; MaRiy{1} = 0; MaRiz{1} = 0;
% Set elements data
...
LNV3DB_main;
clearvars -except edata FEMdata SOLdata time_total time_pre time_solve time_post E0 rho0 L0 mu

%% Compare with literature
% Find analytical normalized natural frequencies 
exact_freqs = zeros(FEMdata.N_modes,1);               
transc_eq = @(x) mu*x-cot(x);           % Transcendental equation from clamped-tipmass longitudinally vibrating beam
...
norm_omega = SOLdata.omega/sqrt(E0/rho0); 
% Relative errors
